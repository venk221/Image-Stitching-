...
from Network.Network_custom_unsupervised import HomographyModel
import cv2
import sys
import os
import numpy as np
...
from Dataset_unsupervised_train import ImageDataset
...
def TrainOperation(
    ...
    for Epochs in tqdm(range(StartEpoch, NumEpochs)):
        ...
        for batch_idx,batch in tqdm(enumerate(train_dataloader),total=len(train_dataloader)):
            ...
            patch_A,patch_B,label,corner_A,corner_B,img_path=batch

            # print(len(corner_A))

            corners_A_list=corner_A[0]+corner_A[1]+corner_A[2]+corner_A[3]
            corner_A=torch.stack(corners_A_list)

            # print(corner_A.shape)

            patch_A=patch_A.to(device)
            patch_B=patch_B.to(device)
            label=label.to(device)
            corner_A=corner_A.to(device)


            # batch=batch.to(device)

            LossThisBatch = model.training_step(patch_A,patch_B,label,corner_A)
        ...
        for batch_idx_val,batch in tqdm(enumerate(validation_dataloader),total=len(validation_dataloader)):
            ...
            validation_loss = model.validation_step(patch_A,patch_B,label,corner_A)
    ...
    with open('training_loss_unsupervised.pkl','wb') as f:
        pickle.dump(train_loss_per_epoch,f)

    with open('validation_loss_unsupervised.pkl','wb') as f:
    ...
    plt.savefig('Train_loss_unsupervised.png')
